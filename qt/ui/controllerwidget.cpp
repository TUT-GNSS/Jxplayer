...
#include "../../player/src/engine/video_pipeline.h"
...
ControllerWidget::ControllerWidget(QWidget* parent, std::shared_ptr<jxplay::IPlayer> player)
  ...
  // 初始化超分辨率按钮
  button_super_resolution_ = new QPushButton("启用超分辨率", this);
  connect(button_super_resolution_, &QPushButton::clicked, this, &ControllerWidget::OnSuperResolutionButtonClicked);

  // 初始化超分辨率模型选择下拉框
  combo_sr_model_ = new QComboBox(this);

  // 获取视频管线
  auto video_pipeline = dynamic_cast<jxplay::VideoPipeline*>(player_->GetVideoPipeline());
  if (video_pipeline) {
    // 获取可用的超分辨率模型
    auto models = video_pipeline->GetAvailableSuperResolutionModels();
    for (const auto& model : models) {
      combo_sr_model_->addItem(QString::fromStdString(model));
    }

    // 如果没有可用模型，添加默认模型
    if (models.empty()) {
      combo_sr_model_->addItem("fsrcnn");
    }

    // 连接模型选择信号
    connect(combo_sr_model_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &ControllerWidget::OnSuperResolutionModelChanged);
  }
  ...
  layout->addStretch();
  layout->addWidget(button_super_resolution_);
  layout->addWidget(combo_sr_model_);
  setLayout(layout);
...
void ControllerWidget::OnPlayButtonClicked() {
  ...
  if (player_->IsPlaying()) {
    ...
    QTimer::singleShot(100, [this]() { player_->Pause(); });
...
void ControllerWidget::OnExportButtonClicked() {
  ...
  if (player_->IsRecording()) {
    ...
    QString dirName = QFileDialog::getSaveFileName(this, tr("选择导出路径"),
...
void ControllerWidget::OnSuperResolutionButtonClicked() {
  if (!player_)
    return;
  ...
  if (button_super_resolution_->text() == "启用超分辨率") {
    video_pipeline->EnableSuperResolution(true);
    button_super_resolution_->setText("禁用超分辨率");
  } else {
    video_pipeline->EnableSuperResolution(false);
    button_super_resolution_->setText("启用超分辨率");
  }
}

void ControllerWidget::OnSuperResolutionModelChanged(int index) {
  if (!player_ || index < 0)
    return;
  ...
  // 设置超分辨率模型
  std::string model_name = combo_sr_model_->itemText(index).toStdString();
  video_pipeline->SetSuperResolutionModel(model_name);
  std::cout << "切换超分辨率模型: " << model_name << std::endl;
}